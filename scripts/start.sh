...
show_help() {
    echo -e "${BLUE}======================================${NC}"
    echo -e "${BLUE}Brick X Clock Service - Start Script${NC}"
    echo -e "${BLUE}======================================${NC}"
    echo -e ""
    echo -e "${GREEN}Purpose:${NC}"
    echo -e "  Start the clock service container"
    echo -e ""
...
start_service() {
    ...
    print_info "Starting Brick X Clock Service..."
    ...
    docker run -d \
        ...
        --health-cmd="curl -f http://localhost:$API_PORT/health" \
        --health-interval=30s \
        --health-timeout=10s \
        --health-retries=3 \
    ...
    print_info "Clock service started successfully!"
    print_info "Container: $CONTAINER_NAME"
    print_info "API Port: $API_PORT"
    print_info "NTP Port: $NTP_PORT"
